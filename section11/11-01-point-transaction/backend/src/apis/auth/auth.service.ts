...
import { User } from "../users/entities/user.entity"; // User 엔티티를 가져옵니다.
...
export class AuthService {
    ...
    // refreshToken을 생성하고 반환하는 로직이 구현
    getRefreshToken({user} : LoginUserDtoGetAccessToken): string {
        return this.jwtService.sign(
            { sub: user.id }, // JWT의 payload에 user.id를 포함합니다.
            { secret: this.configService.get<string>('JWT_REFRESH_SECRET'), expiresIn: '7d' } // 비밀 키와 유효 기간을 설정합니다.
        );
    }
 
    async login({ email, password }: LoginUserDto): Promise<{ accessToken: string, refreshToken: string }> {
        // 1. 이메일로 사용자 찾기
